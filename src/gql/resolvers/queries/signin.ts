...
const comparePasswordAndGetAccessToken = async ({
  password,
  salt,
  hashedPassword,
  username,
...
>) => {
  if (
    timingSafeEqual(
      Buffer.from(await UserCredential.hashPassword(password, salt)),
...
const signin: QueryResolvers["signin"] = {
  resolve: (_, { signinInput }) =>
    getResponse(signinInput).catch(getErrorResponse),
};
