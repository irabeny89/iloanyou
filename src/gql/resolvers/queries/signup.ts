...
const getResponse = async ({ email, password, username, otp }: QuerySignupArgs["signupInput"]) => {
  UserCredential.validatePassword(password);

  const otpPayload = verify(otp, env.jwtSecret!) as OtpPayloadT & JwtPayload

  if (otpPayload.email !== email) throw new Error(EMAIL_MISMATCH_ERRROR_MESSAGE)
  ...
  const userCredential = new UserCredential({ hashedPassword, salt, user: user._id });
  ...
  return {
    ...
    signupReturnData: {
      accessToken: UserCredential.createAccessToken({ userId: user._id+'', username, email, role: userCredential.role as AccessTokenPayloadT["role"] })
    },
  };
}
...
const signup: QueryResolvers["signup"] = {
  resolve: (_, { signupInput }) => getResponse(signupInput)
